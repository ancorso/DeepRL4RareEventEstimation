...
function gradual_target_increase(𝒮, 𝒟; info)
	...
	if 𝒮.agent.π isa MISPolicy && length(trainable_policies(𝒮.agent.π)) > 1
...
function policy_gradient_training(𝒮::EvaluationSolver, 𝒟)
    ...
    # Train Actor
    batch_train!(actor(𝒮.agent.π), 𝒮.a_opt, 𝒮.𝒫, 𝒟, info=info, π_loss=𝒮.agent.π)
    
    # Optionally update critic
    if !isnothing(𝒮.c_opt)
        batch_train!(critic(𝒮.agent.π), 𝒮.c_opt, 𝒮.𝒫, 𝒟, info=info, π_loss=𝒮.agent.π)
    end
...
function value_training(𝒮::EvaluationSolver, 𝒟)
    ...
    for epoch in 1:𝒮.c_opt.epochs
        ...
        if ((epoch-1) % 𝒮.c_opt.update_every) == 0
            Crux.train!(critic(𝒮.agent.π), (;kwargs...) -> 𝒮.c_opt.loss(𝒮.agent.π, 𝒮.𝒫, 𝒟batch, y; kwargs...), 𝒮.c_opt, info=info)
        end
        
        # Train the actor 
        if !isnothing(𝒮.a_opt) && ((epoch-1) % 𝒮.a_opt.update_every) == 0
            Crux.train!(actor(𝒮.agent.π), (;kwargs...) -> 𝒮.a_opt.loss(𝒮.agent.π, 𝒮.𝒫, 𝒟batch; kwargs...), 𝒮.a_opt, info=info)
...
function POMDPs.solve(𝒮::EvaluationSolver, mdp)
	...
	𝒟 = buffer_like(𝒮.buffer, capacity=𝒮.training_buffer_size, device=device(𝒮.agent.π))
    ...
    for 𝒮.i = range(𝒮.i, stop=𝒮.i + 𝒮.N - 𝒮.ΔN, step=𝒮.ΔN)
		...
		episodes!(s, 𝒟, store=𝒮.buffer, Neps=𝒮.ΔN, explore=true, i=𝒮.i, cb=(D) -> 𝒮.post_sample_callback(D, info=info, 𝒮=𝒮))
